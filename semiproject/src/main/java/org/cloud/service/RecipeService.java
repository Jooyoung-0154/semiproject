...
public class RecipeService {
    ...
    public Map<String, Object> searchRecipesPaged(RecipeSearchParams params) {
        ...
        for (Recipe_Info info : recipeInfos) {
            List<Tag> tags = tagMapper.getTagsByRecipeId(info.getRecipeId());
            info.setTags(tags);
        }
    ...
    public String registerRecipe(Recipe recipe) {
        ...
        if (recipe.getTags() != null && !recipe.getTags().isEmpty()) {
            for (Tag tag : recipe.getTags()) {
                tagMapper.insertRecipeTag(newId, tag.getTagId());
            }
        }
    ...
    public List<Recipe_Info> getRecipesByWriterId(String writerId) {
        ...
        for (Recipe_Info recipe : recipes) {
            List<Tag> tags = tagMapper.getTagsByRecipeId(recipe.getRecipeId());
            recipe.setTags(tags);
        }
        return recipes;
    }
